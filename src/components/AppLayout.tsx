...
import { LayoutDashboard, Inventory2Icon, ShoppingBag, User, Menu, Bell } from "lucide-react";
...
const NavIcon = ({ icon: Icon, active }: { icon: any, active: boolean }) => (
  <Icon className={cn("w-6 h-6", active ? "text-primary-container fill-current" : "text-outline")} />
);
...
export const AppLayout = () => {
  ...
  const hideNav = ["/", "/select-role", "/auth", "/verification", "/registration", "/shop-setup", "/welcome"].includes(location.pathname);
          ...
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => cn(
                ...
                isActive ? "text-primary-container scale-105" : "text-outline"
              )}
            >
              {({ isActive }) => (
                <>
                  <item.icon size={24} className={cn(isActive && "fill-current")} />
                  <span className={cn("text-[9px] font-bold uppercase tracking-wider", isActive ? "text-primary-container" : "text-outline")}>
                    {item.label}
                  </span>
                </>
              )}
                 ...
                 <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="bg-primary-container/10 text-primary-container px-2 py-0.5 rounded-full inline-flex items-center gap-1">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest">Pro Seller</span>
                    </div>
                  </div>
                 </div>
                 <button onClick={() => setIsMenuOpen(false)} className="p-2 rounded-full hover:bg-surface-container">
                   <X size={20} />
                 </button>
               </div>

               <div className="space-y-2 flex-1">
                  <SideNavItem icon={LayoutDashboard} label="Dashboard" active onClick={() => setIsMenuOpen(false)} />
                  <SideNavItem icon={PackageIcon} label="My Inventory" onClick={() => setIsMenuOpen(false)} />
                  <SideNavItem icon={ShoppingBag} label="Orders" badge="3" onClick={() => setIsMenuOpen(false)} />
                  <SideNavItem icon={User} label="Profile" onClick={() => setIsMenuOpen(false)} />
                  <div className="h-px bg-outline-variant/20 my-4 mx-2" />
                  <SideNavItem icon={HelpCircleIcon} label="Help & Support" onClick={() => setIsMenuOpen(false)} />
               </div>

               <div className="pt-6 border-t border-outline-variant/20 space-y-6">
                <Button variant="outline" className="w-full h-14 rounded-2xl bg-surface-container-low border-none font-bold text-on-surface">
                  <LogOut className="mr-3 text-error" size={20} /> Logout
                </Button>
                <div className="text-center">
                  <p className="text-[10px] text-outline font-medium tracking-wide">Artisan Hub v2.4.0</p>
                  <div className="flex justify-center gap-4 mt-2">
                    <button className="text-[9px] font-black uppercase tracking-tighter text-primary-container underline">Privacy</button>
                    <button className="text-[9px] font-black uppercase tracking-tighter text-primary-container underline">Terms</button>
                  </div>
                </div>
               </div>
            </motion.div>
...
const SideNavItem = ({ icon: Icon, label, active, badge, onClick }: any) => (
  ...
  >
    ...
    {badge && <span className="ml-auto bg-error text-on-error text-[10px] px-2 py-0.5 rounded-full font-black">{badge}</span>}
