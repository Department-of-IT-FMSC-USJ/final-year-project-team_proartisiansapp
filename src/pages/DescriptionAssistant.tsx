...
export default function DescriptionAssistant() {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      if (nextQ) {
        setMessages((prev) => [
          ...prev,
          { id: Date.now().toString(), role: "assistant", content: nextQ },
        ]);
      } else {
  ...
  const previewText =
    "Experience the touch of nature with this beautiful 180×50cm scarf. Hand-woven from 100% organic cotton and dyed with authentic natural indigo, it's the perfect sustainable statement piece for eco-conscious fashion lovers.";
        ...
        <AnimatePresence mode="wait">
          {!isGenerating ? (
            ...
            >
              <Button
                variant="outline"
                className="flex-1 rounded-2xl border-outline-variant/30 h-14 text-on-surface"
              >
                <Edit3 size={18} className="mr-2" /> Edit
              </Button>
              <Button
                className="flex-[2] rounded-2xl h-14 shadow-primary-glow"
                onClick={() =>
